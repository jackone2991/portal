...
func TestHTTPDraftIsNotFoundToAStranger(t *testing.T) {
	...
	rec := servertest.Do(t, h, stranger, http.MethodGet, "/movies/"+row.ID.String(), "")
	servertest.Problem(t, rec, http.StatusNotFound, "movie/not-found")
	rec2 := servertest.Do(t, h, stranger, http.MethodGet, "/movies/"+uuid.New().String(), "")
	servertest.Problem(t, rec2, http.StatusNotFound, "movie/not-found")
	if rec.Body.String() != rec2.Body.String() {
		t.Fatalf("someone else's and missing must answer identically:\n%s\n%s", rec.Body.String(), rec2.Body.String())
	}
	if rec := servertest.Do(t, h, owner, http.MethodGet, "/movies/"+row.ID.String(), ""); rec.Code != http.StatusOK {
...
func TestHTTPDeleteTwiceIs404(t *testing.T) {
	...
	if rec := servertest.Do(t, h, owner, http.MethodDelete, "/movies/"+row.ID.String(), ""); rec.Code != http.StatusNoContent {
		t.Fatalf("first DELETE = %d (%s)", rec.Code, rec.Body.String())
	}
	servertest.Problem(t, servertest.Do(t, h, owner, http.MethodDelete, "/movies/"+row.ID.String(), ""), http.StatusNotFound, "movie/not-found")
}
