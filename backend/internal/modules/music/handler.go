...
func (h *Handler) CreateTrack(w http.ResponseWriter, r *http.Request) {
	...
	if perr {
		server.BadRequest(w, "invalid audio_asset_id")
		return
	}
	...
	server.JSON(w, http.StatusCreated, trackJSON(t))
}
...
func (h *Handler) GetTrack(w http.ResponseWriter, r *http.Request) {
	...
	server.JSON(w, http.StatusOK, trackJSON(t))
}
...
func parseID(w http.ResponseWriter, r *http.Request, name string) (uuid.UUID, bool) {
	...
	if err != nil {
		server.Problem(w, http.StatusNotFound, "music/not-found", "Not Found", "not found")
		return uuid.Nil, false
	}
	return id, true
}
...
func writeMusicErr(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, ErrNotPublishable):
		server.Problem(w, http.StatusUnprocessableEntity, "music/not-publishable", "Not publishable", "a track needs a ready audio asset to publish")
	case errors.Is(err, ErrInvalidAudioAsset):
		server.Problem(w, http.StatusUnprocessableEntity, "music/invalid-audio-asset", "Invalid audio asset", "the audio must be a ready audio asset you own")
	case errors.Is(err, ErrInvalidCoverAsset):
		server.Problem(w, http.StatusUnprocessableEntity, "music/invalid-cover-asset", "Invalid cover asset", "the cover must be a ready image asset you own")
	case errors.Is(err, ErrValidation):
		server.Problem(w, http.StatusUnprocessableEntity, "music/validation", "Validation error", "the request is invalid")
	case errors.Is(err, ErrBadCursor):
		server.Problem(w, http.StatusBadRequest, "music/invalid-cursor", "Invalid cursor", "the pagination cursor is malformed")
	default:
