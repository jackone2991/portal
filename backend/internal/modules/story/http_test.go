...
func TestHTTPDraftIsNotFoundToAStranger(t *testing.T) {
	...
	row, err := repo.CreateStory(context.Background(), CreateStoryInput{OwnerID: owner, Title: "Draft"})
	if err != nil {
		t.Fatal(err)
	}
	rec := servertest.Do(t, h, stranger, http.MethodGet, "/stories/"+row.ID.String(), "")
	servertest.Problem(t, rec, http.StatusNotFound, "story/not-found")
	rec2 := servertest.Do(t, h, stranger, http.MethodGet, "/stories/"+uuid.New().String(), "")
	servertest.Problem(t, rec2, http.StatusNotFound, "story/not-found")
	if rec.Body.String() != rec2.Body.String() {
		t.Fatalf("someone else's and missing must answer identically:\n%s\n%s", rec.Body.String(), rec2.Body.String())
	}
	if rec := servertest.Do(t, h, owner, http.MethodGet, "/stories/"+row.ID.String(), ""); rec.Code != http.StatusOK {
...
func TestHTTPDeleteTwiceIs404(t *testing.T) {
	h, repo := newHTTP(t)
	owner := uuid.New()
	row, err := repo.CreateStory(context.Background(), CreateStoryInput{OwnerID: owner, Title: "Gone"})
	if err != nil {
		t.Fatal(err)
	}
	if rec := servertest.Do(t, h, owner, http.MethodDelete, "/stories/"+row.ID.String(), ""); rec.Code != http.StatusNoContent {
		t.Fatalf("first DELETE = %d (%s)", rec.Code, rec.Body.String())
	}
	servertest.Problem(t, servertest.Do(t, h, owner, http.MethodDelete, "/stories/"+row.ID.String(), ""), http.StatusNotFound, "story/not-found")
}
