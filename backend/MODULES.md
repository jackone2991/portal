...
9. Write the two HTTP-contract tests every module with a per-id resource keeps — a resource that is not yours reads as 404 (never 403) with the body a missing id gets, and the module's repeatable write twice is idempotent (delete: 204 then 404 as an RFC 7807 problem) — over the real router with fakes, using `platform/server/servertest` (`RequireAuth` / `CurrentUser` for `Deps`, `Do`, `Problem`; the existing `modules/<m>/http_test.go` files are the pattern). `Problem` owns the assertion of which 7807 members are required, so a module test never restates it. A module that reads or writes objects tests over `platform/storage/storagetest` rather than its own `storage.Storage` fake — one owner; the package doc says which of the real store's semantics it keeps, and its tests pin them.
10. Add `internal/modules/<name>/README.md` documenting: what the module owns, which modules it talks to, which events it emits/subscribes. Register the events in `docs/reference/events.md`.
