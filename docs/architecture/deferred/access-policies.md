...
| Wire issuer + verifier + handler in `cmd/api/main.go` | [BUILT] | `account.New(...)` constructed and mounted under `/api/v1`; see `MILESTONE_CHECKS.md` (deleted in `f11cf3f`) |
...
Out of scope for the access-control redesign but listed for completeness; the living build-status tracker is `MILESTONE_CHECKS.md` (deleted in `f11cf3f`) (the original README this section referenced was removed).
