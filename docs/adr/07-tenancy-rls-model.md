...
**Exit (as built):** a request under `/api/v1/…` is tenant-scoped end-to-end through `RequireTenant`; a raw query on `portal_app` cannot read another tenant's rows (tested, in CI since 2026-09-11); there is no `/t/` prefix to make optional; cross-tenant work goes through `ForEachTenant` in the worker, not a BYPASSRLS role.
