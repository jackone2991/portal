// Vietnamese lunar calendar (âm lịch) — Hồ Ngọc Đức's algorithm (amlich.com), the
// de-facto standard used by Vietnamese calendars. All astronomy is computed for
// Vietnam's timezone (UTC+7, `VN_TZ`), which is what defines the Vietnamese lunar
// calendar, independent of the app's display timezone.
...
/** Julian day number of a Gregorian date (dd, mm=1..12, yyyy). */
...
/** Convert a Gregorian date (dd, mm=1..12, yyyy) to the Vietnamese lunar date. */
export function solarToLunar(dd: number, mm: number, yy: number, tz: number = VN_TZ): LunarDate {
