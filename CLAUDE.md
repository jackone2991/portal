...
- **[docs/product/](docs/product/)** — `feature-inventory.md` (decisions `D-1`…`D-41` — cite these IDs), `backlog.md` (the live, triaged open-work list — P0 first), `vision.md`, `specs/` (implementation-ready SPEC-01…09), `briefs/` (per-spec framing, `00`…`09`), `analysis/` (point-in-time audits).
...
- **Cover art is a SECOND pass, not part of the import** (`music:enrich_track`). The zip importer reads tags but skips embedded pictures, because a cover costs an ffmpeg extraction, a second asset ingest, and then a wait for the image pipeline — `music.validateImageAsset` refuses a cover that is not `ready`, so it is the one poll-to-ready loop in this vertical. Inline, that would turn a fast "300 tracks are in" into a slow "300 tracks are still importing". The client asks for it afterwards: `POST /tracks/imports/{id}/enrich` (whole job) or `POST /tracks/{id}/enrich` (one track, works for any track with audio). Enrichment **only fills gaps** — `enrichPatch` never overwrites a title, artist, album or cover that already has a value, because by then the user may have typed one. `mediaapi.OpenOriginal` exists for this: re-reading an asset without leaking media's storage keys. **Gotcha:** the asset lookup must run inside the owner's tenant scope (`assets` is RLS-fenced, an unscoped read answers "asset not found"), but the byte stream it returns must NOT — copying 500 MB has no business holding a transaction open.
- **Catalogue lookup is the only outbound third-party call in the codebase** (`music:lookup_track`, migration 0039) and is **off by default**. It asks MusicBrainz + the Cover Art Archive for what an audio file cannot contain: release year, genre, a high-quality cover. Three things are non-negotiable and live in `musicbrainz.go` rather than in callers: (1) **one request per second globally**, held by a Redis `SET NX PX` slot — a per-process ticker would multiply the rate by the replica count and get the deployment blocked; (2) a **real User-Agent with contact details**, which their policy requires and for which there is deliberately no default — `MUSICBRAINZ_ENABLED=true` without `MUSICBRAINZ_CONTACT` stays off; (3) **no guessing** — below a MusicBrainz score of 88 the result is recorded as `no_match`, because a wrong album silently attached to a track is worse than an empty field and much harder to undo. Disabled, the endpoints answer **503** `music/lookup-disabled` naming the env vars, not 404. Like the other passes it only fills gaps — enforced in SQL this time (`SetTrackLookupResult` COALESCEs every field). The three metadata passes are: **import** (filename) → **enrich** (ffprobe, local) → **lookup** (network, throttled, opt-in).
- **`/assets/{id}/original` must stay a `ServeContent` route** ([backend/internal/modules/media/objectreader.go](backend/internal/modules/media/objectreader.go)). It is the source URL every `<audio>`/`<video>` element plays from, and a browser that cannot range-request its media source reports `seekable` as `[0,0]` and silently refuses **every** seek — the progress bar still animates, the click does nothing, and it reads as a front-end bug. `io.Copy` sends no `Accept-Ranges`, so the fix is structural: `storage.GetByteRange` + a lazy `objectReader` (`io.ReadSeeker`) handed to `http.ServeContent`. "Lazy" is load-bearing — `ServeContent` opens with `Seek(0, End)` to size the body, and a reader that fetched the object to answer that would download the whole file on every scrub. Don't simplify this back into a copy loop.

- **The ledger is icon-first (0042).** `bank_categories` carries an `icon` (an **emoji**, not a sprite id — the Olympus sprite is a social-network set with no money/food/transport glyphs) and a `color` (`#rrggbb`, CHECK-constrained because the value reaches a `style` attribute). Both are NULLable and the UI falls back to a letter chip, so "no icon" is an ordinary state, not a defect. `GET /bank/report` is the month aggregate behind the donut and the trend bars; it **rolls child categories up into their parent in SQL** (`COALESCE(parent_id, id)`) — a breakdown that split "Cà phê" out of "Ăn uống" is a chart of slivers — and excludes **pure** transfer legs while still counting a transfer *fee* (`transfer_id` and `category_id` both set) as an ordinary expense, exactly as `MonthFlowTotals` does. Diverge from that filter in one place and the donut stops summing to the month total.
...
- **RLS is enforced only when the app connects as `portal_app`** (NOSUPERUSER, NOBYPASSRLS — created by migration 0019). `docker-compose.yml` reads `DATABASE_URL` from `.env`; the checked deployment (2026-09-11) points it at `portal_app`, so the tenant policies (`grep -ho 'ALTER TABLE [a-z_]* FORCE ROW LEVEL SECURITY' backend/db/migrations/*.up.sql | sort -u | wc -l` tables — 29 at last check) are live there. Since 2026-09-11 `.env.example` defaults `DATABASE_URL` to `portal_app` too (with the placeholder password migration 0019 seeds), so a fresh `make up` starts enforced; the superuser `portal` is used only by `MIGRATE_DATABASE_URL` (`make migrate` — `portal_app` cannot create tables or policies) and `BACKUP_DATABASE_URL` (`pg_dump` must see every tenant). Verify what a deployment runs as with `grep DATABASE_URL .env`; the cutover steps and the isolation test (`go test ./internal/platform/db -run TestRLS` with `RLS_TEST_ADMIN_URL`/`RLS_TEST_APP_URL`) are [docs/operations/rls-cutover.md](docs/operations/rls-cutover.md). That suite also runs on every push: the `backend` CI job starts a throwaway `postgres:18`, applies the migrations from zero, and points both URLs at it — and under `CI=true` an unpointed suite fails rather than skips, so the gate cannot lapse silently.
...
GitHub Issues on `jackone2991/portal` via `gh`; `docs/product/backlog.md` stays the triaged list and links to the issues it spawns. See `docs/agents/issue-tracker.md`.
